...
REM
REM Simple batch script to create a Visual Studio project using premake5 and the asteroids.lua script.
REM
REM <!-- Copyright (c) 2025 Tyre Bytes LLC - All Rights Reserved -->
REM --------------------------------------------------------------------------------------------------------------------
...
:process_arguments_loop
	if %argumentIndex% LEQ %argumentCount% (
		...
		IF "--development"=="!argumentName!" (
			SET FLAG_BUILD_DEVELOPMENT=1
			SET FLAG_BUILD_ALL=0
			SET FLAG_BUILD=1
		)
...
IF %FLAG_BUILD% EQU 1 (
	IF %FLAG_BUILD_ALL% EQU 1 (
		...
		SET /A FLAG_BUILD_DEVELOPMENT=1
...
IF %FLAG_BUILD% EQU 1 (
	...
	IF %FLAG_WINDOWS% EQU 1 (
		...
		IF %FLAG_BUILD_DEVELOPMENT% EQU 1 (
			ECHO building development of %CD%"/windows/%PROJECT_NAME%.sln"
			msbuild "windows/%PROJECT_NAME%.sln" /property:Configuration=development /property:Platform="x64" !extra_options!
			IF NOT 0 == %errorlevel% (
				ECHO development build windows failed
				IF NOT "%abs_detailed_report_file%" == "" (
					EXIT /B 1
				)
			)
