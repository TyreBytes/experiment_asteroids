...
#
# Automated Build Script for Asteroids to copy executables from build to run area on Linux / macOS during a build.
#
# <!-- Copyright (c) 2025 Tyre Bytes LLC - All Rights Reserved -->
#---------------------------------------------------------------------------------------------------------------------#

export FinalProjectName=Asteroids
...
if [ "tb_linux" == ${tbBuildPlatform} ]; then
	...
	if [ "release" == ${buildConfig} ] || [ "public" == ${buildConfig} ]; then
		...
		mkdir -p "${runDirectory}../${FinalProjectName}.app/Contents/MacOS"
