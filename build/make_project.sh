...
# 2024-08-03: Everything below this point is directly copied in each make_project.sh unless otherwise noted or this
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--dev | --development ) FLAG_BUILD_DEVELOPMENT=true; FLAG_BUILD_ALL=false; FLAG_BUILD=true; shift ;;
...
if [[ "$FLAG_WEB" == true ]]; then
	...
	if [[ "$FLAG_BUILD_DEBUG" == true ]]; then
		...
		# 2024-08-07 Everything below ---THIS LINE HERE--- was from Asteroids (a game / executable project),
		...
		# #emcc debug/objects/**.o -g2 -s LINKABLE=1 -s DEMANGLE_SUPPORT=1 -s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=2 -o debug/asteroids.html --preload-file ../../run/data@/data --shell-file ../web_shell.html
		# emcc debug/objects/**.o -O1 -s LINKABLE=1 -s DEMANGLE_SUPPORT=1 -s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=2 -o release/asteroids.html --preload-file ../../run/data@/data --shell-file ../web_shell.html
		...
		# sed -Ei 's/^.*?src="(.*?\.js)".*?$/script.src="\1";/' debug/asteroids.html
	fi

	if [[ "$FLAG_BUILD_DEVELOPMENT" == true ]]; then
		emmake make -j $FLAG_MAKE_JOBS config=development 2>> "$BUILD_REPORT_FILE"
	fi
	...
	if [[ "$FLAG_BUILD_RELEASE" == true ]]; then
		...
		# emcc release/objects/**.o -O1 -s LINKABLE=1 -s DEMANGLE_SUPPORT=1 -s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=2 -o release/asteroids.html --preload-file ../../run/data@/data --shell-file ../web_shell.html
		# echo Running sed magic from release area.
		# sed -Ei 's/^.*?src="(.*?\.js)".*?$/script.src="\1";/' release/asteroids.html
	fi
...
if [[ "$FLAG_LINUX" == true ]]; then
	...
	if [[ "$FLAG_BUILD_DEVELOPMENT" == true ]]; then
		make -j $FLAG_MAKE_JOBS config=development 2>> "$BUILD_REPORT_FILE"
		[ "$?" == 0 ] && BUILD_SUCCESS=true || BUILD_SUCCESS=false
		FLAG_RUN_CONFIG=develoment
	fi
	if [[ "$FLAG_BUILD_RELEASE" == true ]]; then
...
if [[ "$FLAG_MACOS" = true ]]; then
	if [ $kLinuxPlatform == $currentPlatform ]; then
		...
		if [[ "$FLAG_BUILD_DEVELOPMENT" == true ]]; then
			sed -i 's/\(CC\|CXX\|AR\) = \(clang\|clang++\|ar\)/\1 = x86_64-apple-darwin15-\2/' macos/${PROJECT_NAME}.make
			make -C macos config=development -j $FLAG_MAKE_JOBS
			x86_64-apple-darwin15-strip macos/development/$PROJECT_NAME
		fi
		...
		#
		#   xcodebuild -workspace ./macos/asteroids.xcworkspace -scheme asteroids build
		#   xcodebuild -workspace ./macos/asteroids.xcworkspace -scheme asteroids -configuration release build

		if [[ "$FLAG_BUILD_DEBUG" == true ]]; then
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME} -configuration debug build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_steam -configuration debug build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo -configuration debug build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo_steam -configuration debug build -quiet 2>> "$BUILD_REPORT_FILE"
		fi

		if [[ "$FLAG_BUILD_DEVELOPMENT" == true ]]; then
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME} -configuration development build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_steam -configuration development build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo -configuration development build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo_steam -configuration development build -quiet 2>> "$BUILD_REPORT_FILE"
		fi

		if [[ "$FLAG_BUILD_RELEASE" == true ]]; then
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME} -configuration release build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_steam -configuration release build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo -configuration release build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo_steam -configuration release build -quiet 2>> "$BUILD_REPORT_FILE"
		fi

		if [[ "$FLAG_BUILD_PUBLIC" == true ]]; then
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME} -configuration public build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_steam -configuration public build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo -configuration public build -quiet 2>> "$BUILD_REPORT_FILE"
			xcodebuild -workspace ./macos/${PROJECT_NAME}.xcworkspace -scheme ${PROJECT_NAME}_demo_steam -configuration public build -quiet 2>> "$BUILD_REPORT_FILE"
		fi
	fi
fi
