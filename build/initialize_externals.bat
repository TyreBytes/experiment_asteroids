...
REM 2025-03-26: It is useful to skip initialization of externals for Automated / Nightly Builds unless the project needs
REM   specific commit/branches. By skipping the initialization, the nightlies will not build TurtleBrains/ICE/TrackBuilder
REM   for each individual project. If a project locks into a specific commit/branch it may wish to comment out the check
REM   below to use build/externals. Using "main" may not mean the main branch, but rather the latest version branch.
@REM IF %abs_automating_builds% EQU 1 (
@REM 	EXIT /B 0
@REM )

SET TURTLE_BRAINS_REVISION=release/v0.4.3
SET INTERNAL_COMBUSTION_REVISION=release/v0.4.3
SET TRACK_BUILDER_REVISION=release/v0.2.14
