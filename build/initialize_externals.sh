...
# 2025-03-26: It is useful to skip initialization of externals for Automated / Nightly Builds unless the project needs
#   specific commit/branches. By skipping the initialization, the nightlies will not build TurtleBrains/ICE/TrackBuilder
#   for each individual project. If a project locks into a specific commit/branch it may wish to comment out the check
#   below to use build/externals. Using "main" may not mean the main branch, but rather the latest version branch.
# if [[ "$abs_automating_builds" == true ]]; then
# 	return 0
# fi

TURTLE_BRAINS_REVISION=release/v0.4.3
INTERNAL_COMBUSTION_REVISION=release/v0.4.3
TRACK_BUILDER_REVISION=release/v0.2.14
